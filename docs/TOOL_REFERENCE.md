...
**481 tools** across 14 categories.

**Maturity**: `offline` = pure Python, CI-tested; `simulator` = runs against the simulator, no Altium needed; `live_only` = verified only on live Altium.
...
| `lib_get_pin_list` | readonly | live_only | Get all pins of the current library component. |
...
| `obj_highlight_net` | silent | live_only | Highlight a net by name in the active schematic or PCB document. |
...
| `pcb_run_drc` | silent | simulator | Run Design Rule Check (DRC) on the active PCB. |
